...
class SearchCharacterViewModel @Inject constructor(
    ...
    private val _searchCharacter = MutableStateFlow<ResourceState<
            CharacterModelResponse>>(ResourceState.Empty())
    val searchCharacter: StateFlow<ResourceState<
            CharacterModelResponse>> = _searchCharacter
    ...
    private suspend fun safeFetch(nameStartsWith: String) {
        ...
        try {
            ...
            _searchCharacter.value = handleResponde(response)
        } catch (t: Throwable) {
            when (t) {
    ...
    private fun handleResponde(response: Response<CharacterModelResponse>)
